...
str;
    ...
    public function adminList($name){
        ...
        if(isset($model)){
			...
			if(empty($map)) $map  =  null;
            if(!isset($order))  $order = '';
            $list = $this->lists($model->field($fields),$map,$order);
            $fields = array();
			if (!empty($list_grid) && is_array($list_grid)){
        ...
        if(is_array($list)){$this->assign('_list', $list);}
    ...
    public function config(){
		...
		if($id){
			...
			$name   =   ucfirst(I('name'));
			$addon  =   M('Addons')->where(array('name'=>$name))->find();
		}      
        if(!$addon)
            $this->error('插件未安装');
