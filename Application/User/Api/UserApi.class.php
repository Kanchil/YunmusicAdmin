...
class UserApi extends Api{
     ...
     */
    public function register($username, $password, $email, $mobile = 0){
     ...
     * 重置密码
     */
	public function resetpwd($uid, $password, $data){
        if($this->model->updateUserPassword($uid, $password, $data) !== false){
            $return['status'] = true;
        }else{
            $return['status'] = false;
            $return['info'] = $this->model->getError();
