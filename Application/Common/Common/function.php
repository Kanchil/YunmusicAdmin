...
// +----------------------------------------------------------------------
// OneThink常量定义
const JY_ADDON_PATH 		= './Addons/';
const JYMUSIC_VERSION    	= '1.1.0';
const JY_UPDATE_TIME    	= '20160705';
/**
 * 系统公共库文件
 * 主要定义系统公共函数库
 */
...
function get_genre_ids($id){	
    ...
    if(isset($list[$key])){ //已缓存，直接使用
       $ids = $list[$key];
       	...
       	$ids 	= M("Genre")->where(array("pid"=>$id))->getField("id",true);
		$ids[] 	= $id;
		$list[$key] = $ids;
       S('genre_ids', $list);
    }
...
*获取服务器组
*/
function get_server($id=0) {
	$list = S('serverList');
	if (!$list){
		$list= M('Server')->field(true)->select();
		S('serverList',$list);
	}	
	if (!empty($id)){
		foreach ($list as &$v) {
...
/*获取标签*/
function get_music_tag($mid,$type=1){
	$tagIds	=	M('MusicTag')->where(array('music_id'=>$mid,'type_id'=>$type))->getField('tag_id',true);
	if (!empty($tagIds)){		
		$tags = M('Tag')->where(array('id'=>array('in',$tagIds)))->getField('id,name');
	}else{
		$tags = null;
	}  		
    return $tags;
...
//自定义标签获取音乐封面;
function get_song_cover($arr,$type='artist'){
	
	if ('artist' == $type){
		$path = M('Artist')->getFieldById($arr['artist_id'],'cover_url');
	}
	if('album' == $type){
		$path = M('Album')->getFieldById($arr['album_id'],'cover_url');
	}	
	return !empty($path)? $path : __ROOT__.'/Public/static/images/cover.png';	
}

//获取时间差;
function beforeTime($time){	
	$mtime = NOW_TIME-$time;//获取秒
...
//获取当前用户头像
function get_user_avatar ($uid,$size=128) {
    /* 获取缓存数据 */
    ...
    if(isset($list[$key])){ //已缓存，直接使用
        ...
        $avatar = M('Picture')->where(array('uid'=>$uid,'type'=>5))->field('path,url')->find();		
        if($avatar){
            $pic = !empty($avatar['path'])? $avatar['path'] :$avatar['url'];
            $avatar = $list[$key] = $pic;
            /* 缓存用户 */
            $count = count($list);
            $max   = C('USER_MAX_CACHE');
            ...
            $avatar = $list[$key] = __ROOT__.'/Public/static/images/avatar.png';
        }
        S('user_avatar_list', $list);
    }
    return    substr($avatar, 0, 1 ) == '/'?  'http://' .$_SERVER['HTTP_HOST'] . $avatar :  $avatar;
}
...
function get_user_groups() {
    /* 获取缓存数据 */
    $list = S('user_groups_list');
    /* 查找用户信息 */
    if(empty($list)){ //已缓存，直接使用
		$list = M('MemberGroup')->select();
        S('user_groups_list',$list);
    } 
	return $list;
}
...
function get_group_links() {
	...
	/* 获取缓存数据 */
    $links = S('user_group_links');
    /* 查找用户信息 */
    if(empty($links)){ //已缓存，直接使用
		$map['end_time'] = array('gt',NOW_TIME);
		$map['group_id'] = array('gt',1);
		$list	= M('MemberGroupLink')->field('id',true)->where($map)->select();
		$groups = array_column(get_user_groups(),'id');
		$links = array();
		...
		if(count($list) > 0) {		
			...
			S('user_group_links',$links,24*60*60);
		}     
    } 
	return $links;
}
...
function is_vip ($uid) {
	...
	if(!empty($links['2']) && is_array($links['2'])){
		return in_array($uid, $links['2']);
	}
	return false;
}

//是否是会员
function is_musician ($uid) {
	$links	= get_group_links();
	if(!empty($links['3']) && is_array($links['3'])){
		return in_array($uid, $links['3']);
	}
	return false;
}

/*获取歌曲歌词*/
function get_lrc($id=null,$auto = false) {
	if(!empty($id)) {
		$id 	= is_array($id)? $id['id'] : (int)$id;
		$lrc	=  M('SongsExtend')->getFieldByMid($id,'lrc');		
		return !empty($lrc)?  nl2br($lrc): "";	
	}else{
		return "";
	}		
}
 ...
 * @param int $expire  过期时间 单位 秒
 */
function jy_encrypt ($data,$expire = 0){
	$key	 = 'jy'.C('DATA_AUTH_KEY');
	$expire  = $expire > 0 ? $expire : 7200;
...
//检测是否关注
function is_follow ($uid){	
	$map['uid']			= is_login();
	$map['follow_uid']	= $uid;
	return  M('UserFollow')->where($map)->find();

}
/*判断是否收藏*/
function is_fav($mid=0,$type=1){	
	return is_handle_music("UserFav",$mid,$type);
	//dump(is_handle_music("UserFav",$mid,$type));
	
}

/*判断是否喜欢*/
function is_like($mid=0,$type=1){	
	return is_handle_music("UserLike",$mid,$type);	
}

/*判断是否推荐*/
function is_recommend($mid=0,$type=1){	
	return is_handle_music("UserRecommend",$mid,$type);	
}


/*判断是否操作*/
function is_handle_music($model="UserFav",$mid,$type){
...
/*获取访客*/
function get_visitors($uid,$limit) {
...
/*
*记录用户操作防刷新
*$action 操作字符串  addFollow ，addFav
* 返回操作次数
*/
function offSpite ($action){
	//记录操作并缓存
	//这里根据IP 记录
	$action = 'action_'.$action;
	$spite = session($action);
	if (empty($spite)){//初始化缓存
		$spite  = array();
		$spite['num'] = 1;
		$spite['lock_time'] = time()+60;
		session($action,$spite);
		return true;
	}else{		
		$actioNum = $spite['num'];
		//验证是否被锁
		$lock_time = $spite['lock_time'];
		if( $lock_time > time()+60){	  //检测30分钟锁  		 
			 return false;  //未解锁
		}
		//没有30分钟锁			    	 		    	
		if ($lock_time > time() && $actioNum > 20)  {//1分钟内操作次数  锁30分钟
			$spite['num'] = 1;
			$spite['lock_time']= (time()+90);
			session($action,$spite);
			return false;
		}else if( $lock_time > time()){
			$spite['num'] = ++$actioNum; 
			session($action,$spite);
			return true;    		  		
		}else{
...
/*获取上传所需地址*/
function get_justup_url (){	
	$url =  U('File/upload',array('session_id'=>session_id()));
	return $url;	
}
