...
function write_config($config, $auth){
    if(is_array($config)){
		...
		$home_view 		= file_get_contents(MODULE_PATH . 'Data/home_view.tpl');
		$article_view 	= file_get_contents(MODULE_PATH . 'Data/article_view.tpl');
		$user_view 		= file_get_contents(MODULE_PATH . 'Data/user_view.tpl');
		$mobile_view	= file_get_contents(MODULE_PATH . 'Data/mobile_view.tpl');
        ...
        //写入应用配置文件
		$dirs = array('Avatars','Editor','Import','Music','Picture','QRcode','UserPic','UserUp');
	
        if(!IS_WRITE){
            return '由于您的环境不可写，请到<a href="http://www.jyuu.cn">官网</a>查看手动安装指南！';
        }else{
			if ( !is_writable('./Data') ){ 
				die("Data文件加不可写，请检查！"); 
			} 
			...
			if (!file_exists('./Data/Conf')){ 
					mkdir ('./Data/Conf/');
					chmod('./Data/Conf',0777);
			}; 
			
            if(file_put_contents('./Data/Conf/conf.php', $conf) &&
				file_put_contents('./Data/Conf/home_view.php', $home_view) &&				   
				file_put_contents('./Data/Conf/user_view.php', $user_view) &&
				file_put_contents('./Data/Conf/mobile_view.php', $mobile_view) &&
				file_put_contents('./Data/Conf/article_view.php',$article_view)){					
                show_msg('配置文件写入成功');
            } else {
...
function register_administrator($db, $prefix, $admin, $auth){
    ...
    $sql = "REPLACE INTO `[PREFIX]member` VALUES ".
           "(1, '[NAME]', 0, 0, 0, 0, 0, 0, 1,1,'0000-00-00', 'NULL', 0, 0, 'NULL', 'NULL', 0, 0, 0, 0, 0, [TIME], [TIME], [TIME], [TIME], 1, NULL);";
    $sql = str_replace(
