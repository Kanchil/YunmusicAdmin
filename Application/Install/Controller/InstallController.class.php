        ...
        if(IS_POST){
            ...
            if(!is_array($db) || empty($db[0]) ||  empty($db[1]) || empty($db[2]) || empty($db[3])){
				...
				if (function_exists('mysql_connect')) {
					if(!@mysql_connect($DB['DB_HOST'].":".$DB['DB_PORT'], $DB['DB_USER'], $DB['DB_PWD'])) {
						$this->error('数据库连接失败，请检查数据库配置！');
					}	
				}					
				//检测数据库连接
                $result1 = $db->execute('select version()');
                if (!$result1) {
                    $this->error('数据库连接失败，请检查数据库配置！');
                }
                
				$sql2 = "CREATE DATABASE IF NOT EXISTS `{$dbname}` DEFAULT CHARACTER SET utf8";
				$db->execute($sql2) || $this->error($db->getError());
            }

            //跳转到数据库安装页面
            $this->success("",U('step3'));
        } else {
        	session('update',null);
            if(session('update')){
                session('step', 2);
